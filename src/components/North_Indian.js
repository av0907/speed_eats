...
const North_Indian = () => {
  ...
  const [northIndianhotels, setNorthIndianHotels] = useState([]);
  let json_flitered=[];
  ...
  async function fetchData(){
    ...
    setNorthIndianHotels(json_flitered);

  }
   
  if (northIndianhotels.length===0){
  ...
  return (
    <div className="w-5/6 m-auto flex flex-wrap items-center">
      {northIndianhotels.map((hotel)=><ItemHotel hotel={hotel}/>)}
    </div>
  );
